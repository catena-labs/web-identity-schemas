...
export const vpTypeSchema = (type?: string | string[]) => {
  if (type) {
    const additional = Array.isArray(type) ? type : [type]
    return v.tupleWithRest(
      [VpTypeLiteralSchema, ...additional.map((t) => v.literal(t))],
      v.string(),
    )
  }
  return v.union([
 ...
 */
export const PresentationSchema = v.looseObject({
  /** JSON-LD context */
 ...
 * @see {@link https://www.w3.org/TR/vc-data-model/#presentations}
 */
export const VerifiablePresentationSchema = v.looseObject({
