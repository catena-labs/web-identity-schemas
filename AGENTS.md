...
# Run linting AND type checking (oxlint is type-aware via tsgolint,
# Microsoft's Go TypeScript port — there is no separate `typecheck` script)
pnpm run lint
...
> [!NOTE]
> Type checking is performed by the lint step. `pnpm run lint` runs `oxlint`
> with `"typeAware": true` (see `.oxlintrc.json`), which uses `oxlint-tsgolint`
> (Microsoft's Go implementation of the TypeScript type checker). There is no
> standalone `pnpm run typecheck` — running `pnpm run lint` covers both linting
> and type errors.
...
- Always run `pnpm run lint` before commits — it is type-aware (oxlint +
  tsgolint), so it reports both lint and TypeScript type errors in one pass
